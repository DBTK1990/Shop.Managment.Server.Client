...
export default class AppointmentService {
  ...
  getListByPage(page_number, filter = "date", order = 1) {
    let url = `${this.baseAddress}${this.controllerUri}${this.paths[0]}/${page_number}`;
  ...
  edit(id, appointment_date) {
    ...
    let url = `${this.baseAddress}${this.controllerUri}${this.paths[3]}/${id}`;
  ...
  delete(id) {
    ...
    let url = `${this.baseAddress}${this.controllerUri}${this.paths[4]}/${id}`;
